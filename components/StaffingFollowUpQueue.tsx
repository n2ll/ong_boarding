    ...
    <section id="staffing-followups" aria-labelledby="staffing-followups-title" className="min-w-0 rounded-2xl border border-border-strong bg-card p-4 sm:p-5">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          <h2 id="staffing-followups-title" className="text-base font-bold text-foreground">오늘 후속 연락</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">전체 공고 기준{data ? ` · ${data.today}` : ""}</p>
        </div>
      </div>
        ...
        <>
          ...
          {data.invalid_records > 0 ? (
            <p role="status" className="mt-3 rounded-xl border border-warning/30 bg-warning-soft p-3 text-sm text-warning-strong">
              연락·할 일 기록 {data.invalid_records}건을 확인하지 못했어요. 확인된 항목만 표시합니다.
            </p>
          ) : null}
          ...
          <div role="group" aria-label="후속 연락 예정일 필터" className="mt-4 flex flex-wrap gap-2">
            ...
            >
              전체 예정 {incomplete ? "확인된 " : ""}{data.items.length}건
            </Button>
          </div>
          ...
          {items.length === 0 ? (
            <p role="status" className="mt-4 text-sm text-muted-foreground">
              {incomplete
                ...
                : filter === "due"
                  ? "오늘 또는 기한이 지난 후속 연락이 없어요."
                  : "예정된 후속 연락이 없어요."}
              {filter === "due" && data.items.length > 0 ? " 이후 일정과 예정일 미정 항목은 전체 예정에서 확인하세요." : ""}
            </p>
          ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex min-w-0 flex-wrap gap-1.5 text-xs">
                        ...
                        <span className={`rounded-full px-2 py-1 ${isOverdue ? "bg-priority-critical-soft text-priority-critical-ink" : isToday ? "bg-priority-attention-soft text-priority-attention-ink" : "bg-muted text-muted-foreground"}`}>
                          {item.due_date ? `${isOverdue ? "기한 지남" : isToday ? "오늘" : "예정"} · ${item.due_date}` : "예정일 미정"}
                        </span>
                      </div>
                    </div>
                    <Button
                      type="button"
                      size="sm"
                      aria-label={`${item.name || "이름 미상"} · ${item.job_title} 연락·기록 열기`}
                      onClick={() => onOpen(item)}
                      className="w-full sm:w-auto"
                    >
                      연락·기록 열기 <ArrowRight aria-hidden="true" size={14} />
                    </Button>
                  </li>
                );
              })}
