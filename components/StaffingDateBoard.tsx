...
function DateCell({ cell, title, onOpen }: { cell: StaffingDateBoardCell; title: string; onOpen: () => void }) {
  return <button type="button" aria-label={`${title} ${cell.date} 후보 확인`} onClick={onOpen}
    ...
    <span className={`text-base font-bold ${cell.shortage ? "text-warning-strong" : "text-foreground"}`}>
      {cell.target === null ? "모집목표 미정" : cell.invalid_records ? "기록 확인 필요" : cell.shortage ? `${cell.shortage}명 부족` : "모집목표 충족"}
    </span>
    <span>{cell.target === null ? "목표 미정" : `목표 ${cell.target}명`} · 확정 {cell.confirmed}명</span>
...
export function StaffingDateBoard({ defaultStart, onOpenCandidates }: { defaultStart?: string; onOpenCandidates: (jobId: number, date: string) => void }) {
  ...
  return <section aria-label="날짜별 충원 현황" className="min-w-0 rounded-panel border border-border-strong bg-card p-3 sm:p-4">
    <button type="button" aria-label="날짜별 충원판" aria-expanded={open} aria-controls="staffing-date-board-content" onClick={toggle}
      ...
      <span><span className="block font-bold">날짜별 충원판</span><span className="block text-sm text-muted-foreground">라인과 날짜를 고르면 후보 정리로 이어집니다.</span></span>
    ...
    {open && <div id="staffing-date-board-content" className="mt-4 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">공고 모집인원 기준 · 최대 7일 비교. 실제 운행 여부와 날짜별 필요 인원은 별도로 확인하세요. 관리자가 날짜별로 확정한 사람만 충원에 반영하며, 예비 후보는 포함하지 않습니다.</p>
          ...
          {!current.jobs.length ? <p className="py-6 text-sm">비교할 모집 중인 일반 배송 공고가 없습니다.</p> : <>
            <div className="hidden max-w-full overflow-x-auto lg:block">
              <table className="w-full border-separate border-spacing-2 text-left text-sm">
                <caption className="sr-only">공고 모집인원을 기준으로 비교한 날짜별 확정·예비·부족 인원</caption>
                ...
                <tbody>{current.jobs.map((job) => <tr key={job.job_id}>
                  ...
                  {job.cells.map((cell) => <td key={cell.date} className="align-top"><DateCell cell={cell} title={job.title} onOpen={() => onOpenCandidates(job.job_id, cell.date)} /></td>)}
                </tr>)}</tbody>
              </table>
            </div>
            <div className="space-y-4 lg:hidden">{current.jobs.map((job) => <article key={job.job_id} className="space-y-2">
              ...
              <div className="grid gap-2 sm:grid-cols-2">{job.cells.map((cell) => <DateCell key={cell.date} cell={cell} title={job.title} onOpen={() => onOpenCandidates(job.job_id, cell.date)} />)}</div>
            </article>)}</div>
          </>}
        </>}
    </div>}
