...
export function InterestQueueCard({ initialJobId }: { initialJobId?: number | null } = {}) {
  ...
  const collapsed = !error && !!data && count === 0;
  ...
  return (
    ...
    >
      ...
      {error ? (
        ...
        <div className="mt-1.5 text-[12px] text-muted-foreground">처리 대기 중인 관심 표시가 없어요 · 후보가 맞춤 공고 링크에서 관심을 누르면 여기가 펼쳐집니다.</div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-wrap px-3 pt-3 pb-1.5">
                  ...
                  {rows.some((r) => r.immediate) && (
                    <span className="flex items-center gap-0.5 text-[11px] font-bold text-success-strong">
                      <Zap size={11} /> 바로 가능
                    </span>
                  )}
                  ...
                  {head.phone && (
                    <a
                      ...
                      className="inline-flex items-center gap-0.5 text-[12px] font-semibold text-info hover:underline rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                  ...
                  return (
                    <div key={it.candidate_id} className="flex items-center gap-2 mx-2 mb-2 px-2.5 py-2 rounded-lg border border-muted bg-white flex-wrap">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-[12px] font-bold text-white bg-foreground hover:bg-gray-800 px-3 py-1.5 rounded-lg shrink-0 transition-colors disabled:opacity-60 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
                        <Send size={13} /> 빠른 컨택
                      </button>
                      <button
                        ...
                        className="flex items-center gap-1 text-[12px] font-bold text-gray-700 bg-white border border-border-strong hover:bg-background px-3 py-1.5 rounded-lg shrink-0 transition-colors disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
                      ...
                      <button
                        ...
                        disabled={busy}
                        title="보류"
                        className="flex items-center gap-1 text-[12px] font-bold text-muted-foreground bg-white border border-border-strong hover:bg-background hover:text-error px-2.5 py-1.5 rounded-lg shrink-0 transition-colors disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-error/40"
                      >
                        <XCircle size={13} /> 보류
                      </button>
                    </div>
