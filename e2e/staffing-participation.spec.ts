...
for (const width of [1280, 390]) test(`실제 선탑·백업 참여를 기록하고 정정·삭제 이력을 보존한다 ${width}px`, async ({ page, baseURL }) => {
  ...
  const edit = page.getByRole("button", { name: "가상참여후보 진행 기록", exact: true });
  await edit.click();
  await page.getByRole("dialog", { name: "가상참여후보 진행 기록", exact: true }).getByRole("button", { name: "실제 참여", exact: true }).click();
  const editor = page.getByRole("dialog", { name: "가상참여후보 진행 기록", exact: true });
