...
test("연결 후보와 관심 구분, 날짜별 준비 저장 실패 재시도와 모바일 편집", async ({ page }) => {
  ...
  const dialog = page.getByRole("dialog", { name: "가상후보1 진행 기록", exact: true });
  ...
  const button = dialog.getByRole("button", { name: "진행 기록 저장" });
...
for (const width of [1280, 390]) test(`수신 답변 제안 확인 반영과 본담당 후보 중복 경고 ${width}px`, async ({ page, baseURL }) => {
  ...
  await expect(dialog.getByRole("button", { name: "진행 기록 저장" })).toBeEnabled();
  ...
  await page.getByRole("button", { name: "가상후보2 진행 기록", exact: true }).click();
  await page.getByRole("dialog", { name: "가상후보2 진행 기록", exact: true }).getByRole("button", { name: "투입 날짜", exact: true }).click();
  const ambiguous = page.getByRole("dialog", { name: "가상후보2 진행 기록", exact: true });
...
for (const width of [1280, 390]) test(`선탑 후 본인 의사와 팀 기록 이력, 충돌한 내 초안 보존 ${width}px`, async ({ page, baseURL }) => {
  ...
  await dialog.getByRole("button", { name: "진행 기록 저장", exact: true }).click();
  ...
  await expect(dialog.getByRole("button", { name: "진행 기록 저장", exact: true })).toBeDisabled();
  ...
  await expect(dialog.getByRole("button", { name: "진행 기록 저장", exact: true })).toBeInViewport();
