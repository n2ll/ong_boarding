...
export type AutomationAiMetric = {
  ...
  value: "확인 중" | "확인 실패" | "갱신 실패" | "자동 응대" | "코파일럿" | "전역 중지" | "테스트 1명만 자동 응대" | "선택 대상만 자동 응대";
  detail: string | null;
...
export function automationOverview(input: AutomationOverviewInput): AutomationOverview {
  ...
  const pilotSession = modeView.state === "ready" ? modeView.pilotSession : undefined;
  ...
  const ai: AutomationAiMetric = {
    ...
    detail: modeView.state === "error"
      ...
      : modeView.state === "stale"
        ...
        : pilotSession ? `선택 ${pilotSession.applicant_ids.length}명·${pilotSession.job_ids.length}공고만 새 답장에 응대합니다. 예약 작업은 중지 상태입니다.`
