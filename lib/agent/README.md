...
### 관리자 선탑 진행 문맥
`consultation-context.ts`는 노출을 통과한 활성 후보 연결의 최신 `staffing_preparation`만 읽어 선탑 상태·시간 확인 여부·실제 참여·연락 이력 유무로 축약한다. 예정 상태의 개인 선탑 일시와 경과 여부, 내부 후속 업무의 기한 상태도 전달한다. 내부 메모/상세 장소/담당자/연락 결과 원문은 모델에 전달하지 않는다. 조회 오류·최신 손상은 자동 응대를 보류한다.
현재 일반 라인에 선탑 진행 근거가 있으면 router가 `processConsultation`을 사용한다. 공개 조건 답변·신규 원문 관찰은 유지하고 체크리스트를 다시 시작하지 않는다. `training-followup.ts`는 해당 공고의 이미 확인한 선탑 질문을 생략한다. 본인의 미래 선탑 일시 재확인은 서버가 관리자 기록으로 답하며, 지난 일정·미기록·마감 공고·변경/취소는 매니저에게 인계한다. 내부 후속 업무 기한을 연락 약속으로 안내하거나 선탑 일시·연락 이력으로 배정을 추정하지 않는다. 검수: [관리자 일정 존중](../../.ai/manager-context-followup/eval.md).
