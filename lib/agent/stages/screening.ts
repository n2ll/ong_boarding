...
- 호칭 "[이름]님" / "선생님". 이름은 반드시 성을 포함한 전체 성명으로 ("송시권님" ⭕ / "시권님" ❌ — 성을 빼고 부르지 마라). 톤 친근. 묶음 메시지는 4~6줄 OK.
...
시스템이 매니저 인계 큐에 남기고, Slack 알림이 켜져 있으면 함께 알린다. AI가 한마디 더 보태면 상황이 더 꼬인다.
...
// ─────────────────────────────────────────────────────────────
// 일반 배송 라인(비마트 외 실제 공고 — 도시락 라인 등) 전용 프롬프트.
...
## 🚨 즉시 pause (매니저 인계) — 다음 신호가 있으면 한 턴이라도 더 응대하지 말고 pause
**pause를 결정했으면 reply_text는 빈 문자열로 두라.** 시스템이 매니저 인계 큐에 남기고, Slack 알림이 켜져 있으면 함께 알린다.
...
## 톤
정중하고 간결하게. 1~3문장. 호칭 "[이름]님" / "선생님" — 이름은 반드시 성을 포함한 전체 성명으로 ("송시권님" ⭕ / "시권님" ❌). 이모지는 가끔만 😊.
 ...
 *
 * 2026-08-11 실측으로 확인됨(조립된 프롬프트를 직접 찍고, 같은 프롬프트로 Claude를 4회 호출):
 *   · 공고 pay_info 있음 → 프롬프트에 '익월' 0회. 답변도 "주급 정산이에요, 정확한 지급일은 매니저가
 *     안내드릴게요"까지만("5일인가요?"로 유도해도 확정하지 않는다).
 *   · 공고 pay_info 없음 → 예전처럼 익월 5일로 답한다(그때는 그게 유일한 근거라 정상).
 * 8-10 스모크에서 답변에 익월이 남았던 것은 이 코드가 아니라 그때 dev 빌드가 변경 전이었던 것으로 보인다.
 */
/**
...
// 일반 배송 라인 전용 tool — 관리 항목을 3키로 좁히고 수집값(collected)을 추가.
