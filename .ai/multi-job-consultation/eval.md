...
## 실제 연속 수신 후 추가 발견
- 18:00:31 / 18:00:36 KST 답장 두 건이 각각 한 번 저장됐고 webhook 처리는 18:00:34 / 18:00:37에 시작됐다. 관리자 화면에 두 원문이 표시되며 job_id는 모두 null이었다.
- 오타·띄어쓰기 생략이 있는 실제 원문에 대해 로컬에서 작성한 해석 결과를 서버 원문 검증기에 넣어 공고별 관심/가능 시간 2건을 검증했다. 외부 모델 평가와 자동 관찰 저장은 수행하지 않았다. 실제 답장 내용의 추가 Claude 평가는 자동 승인 검토가 명시적 전송 승인 부족으로 차단했다.
- 관리자 화면에 전역 가용성 변경 이벤트가 보여 원인을 추적했다. 활성 후보가 없으면 off/수동 보류/모호한 공고 여부와 무관하게 별도 Haiku 분류기를 호출하는 기존 웹훅 경로였다. 이 경로는 실제로 두 문자에 대해 수행됐으며, 앞서 차단된 추가 검수 호출과 구분한다.
