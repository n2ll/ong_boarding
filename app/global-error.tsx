...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <html lang="ko">
      <body style={{ margin: 0, fontFamily: "-apple-system, sans-serif", background: "#F4F2EC", color: "#1C1917" }}>
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 32, textAlign: "center" }}>
          ...
          <p style={{ maxWidth: 420, fontSize: 14, lineHeight: 1.6, color: "#6B655C", margin: 0 }}>
            일시적인 오류일 수 있어요. 다시 시도해 주세요.
            {error.digest ? ` (오류 코드: ${error.digest})` : ""}
          </p>
          <button
            onClick={reset}
            style={{ minHeight: 44, borderRadius: 12, background: "#1C1917", color: "#fff", border: 0, padding: "0 20px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}
          >
            다시 시도
          </button>
        </div>
